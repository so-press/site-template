...
function loadJsonData(filePath) {
  ...
  if (data.fields) {
    ...
    for (const fieldSlug in fields) {
      ...
      data[fieldSlug] = loremGen(field.lorem.units, field.lorem.min, field.lorem.max || false)
    }
  }
...
function hbsCompile(filePath, args = {}) {
  try {
    ...
    content = content.replaceAll('{{> @partial-block }}', '{{#if ' + slug + '.demo }}{{' + slug + '.demo }}{{else}}{{#if ' + slug + '.description }}{{' + slug + '.description }}{{else}}{{' + slug + '.name }}{{/if}}{{/if}}')
    const data = args.data || {}

    if (args.layout || {}) {
      const headContent = fs.readFileSync(
        path.join(globalsBase, 'head.hbs'),
        'utf8'
      )
      const footContent = fs.readFileSync(
        path.join(globalsBase, 'foot.hbs'),
        'utf8'
      )
      content = [headContent, content, footContent, '<script src="/admin/js/ws.js?' + Math.random() + '"></script>'].join('\n')
    }
    ...
    const htmlContent = template(data) // Si des données sont nécessaires, les passer ici

    const htmlBeautifiedContent = beautifyHtml(htmlContent, {
      indent_with_tabs: true, // Utiliser des tabulations pour l'indentation
      indent_size: 1, // Taille d'une tabulation
      wrap_line_length: 80, // Limite de la longueur des lignes
      preserve_newlines: true, // Conserver les nouvelles lignes existantes
      end_with_newline: true // Ajouter une nouvelle ligne à la fin
    })
...
/**
 * Replaces pseudo-tags <[slug]>...</[slug]> with Handlebars {{#slug}}...{{/slug}} only if the slug is in Handlebars.partials.
 * @param {string} input - The input string containing pseudo-tags.
 * @return {string} - The string with pseudo-tags replaced by Handlebars blocks.
 */
function replacePseudoTags(input) {
  for (const slug of allComponents) {
    const regex = new RegExp(`<${slug}>([\\s\\S]*?)<\\/${slug}>`, 'g')
    input = input.replace(regex, `{{#>${slug}}}$1{{/${slug}}}`)
  }
