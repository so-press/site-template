...
import {helpers, registerHelpers} from './hbs-helpers.js'
import {App} from './app.js'
import {cleanPath, randInt} from './utils.js'
...
export const compileHbsFiles = () => {
  // const res = expandLoops(`       <loop times="2 to 4">
  //           <section-items href="/articles.html" title="{{lorem 'words' 2 3}}">
  //               <loop times="3 to 7">
  //                   <liste-item>
...
export const prepareHbs = () => {
  ...
  for (const filePath of hbsFiles) {
    ...
    hbs.registerPartial(partialName, prepareHbsContent(partialContent))
  }
...
function expandLoops(input) {
  const makeInnermostRegex = () =>
    /<loop\b[^>]*\btimes\s*=\s*(["'])(\d+(?:\s+to\s+\d+)?)\1[^>]*>(\s*(?:(?!<loop\b)[\s\S])*?)<\/loop>/g
  ...
  while (changed) {
    ...
    out = out.replace(makeInnermostRegex(), (match, _q, range, body) => {
      const parts = range.trim().split(/\s+to\s+/i)
      let min
      let max

      if (parts.length === 1) {
        max = Number.parseInt(parts[0], 10)
        min = max
      } else {
      ...
      return Array.from({length: count}, (_, i) => body.replaceAll('{{loop.index}}', i + 1)).join('')
    })
...
function prepareHbsContent(content) {
  content = expandLoops(content)
  content = replacePseudoTags(content)
  return content
}
...
function hbsCompile(filePath, args = {}) {
  ...
  const preparedContent = prepareHbsContent(content)
  if (filePath.includes('article-card')) {
    console.log({preparedContent})
  }

  try {
    const template = hbs.compile(preparedContent)
...
/**
 * Remplace les pseudo-tags <[slug] ...>...</[slug]> par des blocs HBS {{#>slug ...}}...{{/slug}}
 * et convertit les attributs contenant des helpers connus.
 *
 * @param {string} input - Chaîne d’entrée avec pseudo-tags.
 * @return {string} Chaîne transformée.
 */
function replacePseudoTags(input) {
  // Liste des noms de helpers connus
  const helperNames = new Set(helpers.map(h => h.name))

  for (const slug of allComponents) {
    const regex = new RegExp(
      `<${slug}([^>]*)>` // groupe 1 = attributs
      + '([\\s\\S]*?)' // groupe 2 = contenu
      + `<\\/${slug}>`,
      'g'
    )

    input = input.replace(regex, (match, attributes, content) => {
      const convertedAttributes = attributes
        ? ' ' + attributes
          // Convertit machin="{{truc}}" selon que 'truc' est un helper connu
          .replaceAll(/(\S+)=["']{{\s*([^}]+)\s*}}["']/g, (m, attrName, expr) => {
            const parts = expr.trim().split(/\s+/)
            const first = parts[0]
            return helperNames.has(first)
              ? `${attrName}=(${expr.trim()})`
              : `${attrName}=${expr.trim()}`
          })
      ...
      return `{{#>${slug}${convertedAttributes ? ' ' + convertedAttributes : ''}}}${content}{{/${slug}}}`
    })
  }
