...
export function registerHelpers() {
  ...
  /**
 * Helper HBS « loop » : exécute un bloc un nombre de fois fixe ou aléatoire.
 *
 * Utilisations prises en charge :
 *  - {{#loop 3}}...{{/loop}}                      → boucle 3 fois.
 *  - {{#loop min=1 max=3}}...{{/loop}}            → boucle entre 1 et 3 fois (aléatoire).
 *  - {{#loop "1 to 3"}}...{{/loop}}               → idem (chaîne « 1 to 3 »).
 *  - {{#loop 1 3 random=true}}...{{/loop}}        → deux nombres + aléatoire.
 *
 * @param {number|string} [a]   Compteur fixe (nombre) ou chaîne de plage "x to y".
 * @param {number}        [b]   Borne max si a et b sont numériques.
 * @param {object}        options Options Handlebars (fourni automatiquement).
 * @return {Handlebars.SafeString} Sortie concaténée du bloc répété.
 */
  hbs.registerHelper('loop', (...args) => {
