...
import {getJson, getFileHash} from './utils.js'
...
export const generateFilesJson = () => {
  ...
  const assetsJsonPath = path.join(distBase, 'assets.json')
  fs.writeFileSync(assetsJsonPath, JSON.stringify({
    css: 'style.css?' + getFileHash(distBase, 'style.css'),
    js: 'scripts.js?' + getFileHash(distBase, 'scripts.js')
  }, null, 2), 'utf8')
}
...
export async function emptyDistFolder() {
  let attempts = 5
  if (!fs.existsSync(distBase)) {
    console.error('The distBase folder does not exist:', distBase)
    return
  }
