...
export async function gen() {
  ...
  if (selectedIndex >= 0 && selectedIndex < options.length) {
    ...
    const tagContent = addSlot.toLowerCase() === 'n' ? `${type} - ${name}` : '{{> @partial-block }}'
    ...
    console.log(`You are going to create a ${type} named "${name}" with the description: "${description}".`)
    ...
    fs.writeFileSync(path.join(newFolderPath, `${name}.hbs`), `<div class="${name}" data-type="${type}">${tagContent}</div>`)
    ...
    fs.writeFileSync(path.join(newFolderPath, `${name}.json`), JSON.stringify(jsonContent, null, 2))

    // Conditionally create the .js file
    if (addJSFile.toLowerCase() !== 'n') {
